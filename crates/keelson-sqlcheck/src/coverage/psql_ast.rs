//! The psql parse-tree walker: one statement's tree → the construct ids it
//! exercised.
//!
//! Eleven hundred lines, and nearly all of it is the inventory rather than
//! the machinery: `PSQL_DETECTABLE` names every construct the manifest may
//! declare, and [`detect`] has one arm per grammar node saying which fields
//! discriminate it. Its own file because [`coverage`](super) is *what is
//! measured and how the gate reports it*, and this is *what PostgreSQL's
//! parse tree looks like* — a reader is nearly always after one or the
//! other.
...
pub(super) const PSQL_ACCOUNTED_KINDS: &[&str] = &[
    // detected
    "SelectStmt",
    "InsertStmt",
    "UpdateStmt",
...
pub(super) fn scan_keywords(sql: &str, seen: &mut BTreeSet<String>) {
