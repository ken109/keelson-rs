...
///
/// Mods are the whole composition story: `psql::select((a, b, c))` is one mod
/// made of three, applied left to right. `apply` consumes `self`, so a mod can
/// hand owned data straight to the query without cloning.
pub trait Mod<Q> {
...
///
/// The building block every `sm::*` / `im::*` helper is written in terms of:
/// `pub fn where_<Q: HasWhere>(e: impl Expression + 'static) -> impl Mod<Q>`
/// returns one of these.
pub struct ModFn<F>(F);

/// Wrap a closure as a [`Mod`].
///
/// Intentionally unbounded: the query type comes from the [`Mod`] impl at the
/// use site, so an inline `|q: &mut SelectQuery|` needs no turbofish.
...
///
/// bob calls these contextual mods. They exist for things that cannot be decided
/// at assembly time — a `WHERE` that depends on the schema in use, say — and
/// they run on every build, so a query keeps them as
/// `Vec<Arc<dyn BuildMod<Q>>>` and applies them to a clone of itself at the top
/// of `write_sql`.
///
/// `&self` rather than `self`, because unlike a [`Mod`] they are applied more
/// than once. Same `Debug + Send + Sync` reasoning as
...
impl<Q, T: BuildMod<Q> + ?Sized> BuildMod<Q> for std::sync::Arc<T> {
    fn apply(&self, q: &mut Q) -> Result<()> {
        (**self).apply(q)
    }
}
...
mod tests {
    ...
    /// Stand-in query: mods append their marker.
    ...
    // `then` not `then_some`: the point is to exercise the idiom the design doc
    // documents, and a real mod is a function call that must stay unevaluated.
    ...
    fn build_mods_run_repeatedly_and_can_fail() {
        let mods: Vec<std::sync::Arc<dyn BuildMod<Q>>> =
            vec![std::sync::Arc::new(AppendAtBuild("once"))];
