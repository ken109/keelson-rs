...
/// backend, which recovers the concrete type with [`Self::as_any`].
...
/// arguments — `Value::I32(100)` becomes `100`, not `{"I32":100}`.
...
mod tests {
    ...
    fn custom_values_serialise_through_their_plain_form() {
        assert_eq!(
            json(Value::Custom(Arc::new(Point(1, 2)))),
            serde_json::json!("(1,2)")
        );
        assert_eq!(
            json(Value::Custom(Arc::new(Opaque))),
