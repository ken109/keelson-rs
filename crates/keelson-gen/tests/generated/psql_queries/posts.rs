...
//! Generated from `tests/queries/psql/posts.sql`. Each query has two faces: a query object that runs the file's own SQL, and a mod that merges the same clauses into a host statement. Both slice the text below, so they can never disagree.
...
/// `comments_with_prefixed_author`'s nested `author`.
#[derive(Debug, Clone, PartialEq)]
pub struct CommentsWithPrefixedAuthorAuthor {
    /// `author_id` — never NULL by rule N2.
    pub id: i32,
    /// `author_name` — never NULL by rule N2.
    pub name: String,
}
/// One row of `comments_with_prefixed_author`.
#[derive(Debug, Clone, PartialEq)]
...
/** `comments_with_prefixed_author` as a query object: the file's own SQL, run as written.
...
///
/// Build `comments_with_prefixed_author` without running it.
...
/// are one nested `author` here.
///
/// Run `comments_with_prefixed_author` and return every row.
...
/** `comments_with_prefixed_author` as a mod: its clauses merged into the host statement, flat.
