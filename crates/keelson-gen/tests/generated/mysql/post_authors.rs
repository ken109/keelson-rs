...
impl keelson_models::View for PostAuthors {
    ...
    fn base_select() -> Self::Select {
        keelson_mysql::select((
            ...
            keelson_mysql::select::from(keelson_mysql::quote("post_authors")),
        ))
    }
}
                ...
                keelson_mysql::select::left_join(keelson_mysql::quote("users"))
                    .on(
                        keelson_mysql::quote(("users", "id"))
                            .eq(keelson_mysql::quote(("post_authors", "user_id"))),
                    ),
...
pub mod then_load {
    ...
    > {
        keelson_models::ThenLoad::new(
            |rows: &[super::PostAuthor]| rows.iter().map(|r| r.user_id).collect(),
    ...
    > {
        keelson_models::ThenLoad::new(
            |rows: &[super::PostAuthor]| rows.iter().map(|r| r.post_id).collect(),
